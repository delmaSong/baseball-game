...
public class TeamCompetition {
	...
	public String countHitRate(double h) {
		...
		if(0.0 < ranNum && ranNum <= 0.1) {
			result = "아웃";
			out += 1;
		}else if(0.1< ranNum && ranNum < (0.1+h)){
			result = "안타";
			anta += 1;
			if (antaFlag) { point += 1; }
		}else if((0.1+h) <= ranNum && ranNum <= ((1 - h) / 2 - 0.05)+0.1+h) {	
			result = "스트라이크";
			strike += 1;
		}else if(((1 - h) / 2 - 0.05)+0.1+h <= ranNum && ranNum < 1.0) {
			result = "볼";
			ball += 1;
		}
	...
	public void printScoreboard() {
		...
		System.out.println("|  "+teamInfo.get(0).teamName+"               |           |");
		System.out.println("|  "+teamInfo.get(1).teamName+"               |           |");
		System.out.println("|                                    |");
		System.out.println("|  "+teamInfo.get(0).teamName+"                       "+teamInfo.get(1).teamName+"    |");
		for(int i=0; i<9; i++) {
			String[] info1 = teamInfo.get(0).memberInfo.get(i).split(", ");
			String[] info2 = teamInfo.get(1).memberInfo.get(i).split(", ");
			System.out.println("|  "+(i+1)+". "+info1[0]+"                        "+(i+1)+". "+info2[0]+"         |");
